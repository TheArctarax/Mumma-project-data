...
# Model with memory 
def memory_model(mass_total, s1z, s2z, spin_1, spin_2, ,,
                distance, inc, phase, lambda):
  ...
  # waveform
  plus = oscillitory['plus'][:] + lambda*memory['plus'][:]
  cross = oscillitory['cross'][:] + lambda*memory['cross'][:]
...
injection_parameters = dict(total_mass=60, spin_1=[0., 0., 0.], spin_2=[0., 0., 0.], distance=600., inc=np.pi()/2, phase=1.3, geocent_time=0)
...
priors['lambda'] = bilby.core.prior.Uniform(0, 1, 'lambda')
