...
def time_domain_memory(
    ...
    if h_lm is not None and times is not None:
        ...
        all_keys = inspect.getargspec(waveforms.surrogate.Surrogate.__init__)[0]
        model_kwargs =\
            {key: kwargs[key] for key in all_keys if key in kwargs}
        wave = waveforms.surrogate.Surrogate(
        ...
        all_keys = inspect.getargspec(waveforms.approximant.Approximant.__init__)[0]
        model_kwargs =\
            {key: kwargs[key] for key in all_keys if key in kwargs}
        wave = waveforms.approximant.Approximant(
            q=q, name=model, total_mass=total_mass, spin_1=spin_1, spin_2=spin_2,
            distance=distance, times=times, **model_kwargs)
    elif model == 'MWM':
        all_keys = inspect.getargspec(waveforms.mwm.MWM.__init__)[0]
        model_kwargs = {key: kwargs[key] for key in all_keys if key in kwargs}
        wave = waveforms.mwm.MWM(
