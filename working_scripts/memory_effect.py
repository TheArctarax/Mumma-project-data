...
for ii, model in enumerate(['IMRPhenomD', 'SEOBNRv4', 'NRSur7dq2']):
	delta_t = 1.0 / 4096
	times = np.linspace(0, delta_t , 2)
        if 'NRSur' in model:
            h_mem, times = gwmemory.time_domain_memory(model=model, q=q, spin_1=S1, spin_2=S2, total_mass=60.,distance=400., inc=np.pi/2, phase=0)
        elif 'IMR' or 'EOBNR' in model:
